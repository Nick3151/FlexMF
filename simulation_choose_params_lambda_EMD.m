...
times = cell(nlambdas, nSim);
emds_W = cell(nlambdas, nSim);
emds_H = cell(nlambdas, nSim);
recon_costs = zeros(nlambdas, nSim);   
reg_costs = zeros(nlambdas, nSim);
constraints = zeros(nlambdas, nSim);
num_detected = cell(nlambdas, nSim);
num_significant = cell(nlambdas, nSim);
W_hats = cell(nlambdas, nSim);
H_hats = cell(nlambdas, nSim);
Ws = cell(nSim, 1);
Hs = cell(nSim, 1);
Xs = cell(nSim, 1);
Rs = cell(nlambdas, nSim);
Ms = cell(nlambdas, nSim);
...
for n=1:nSim
    ...
    Xs{n} = X;
    L = size(W,3);
    parfor Li=1:nlambdas
        lambda = lambdas(Li);
        display(['Testing lambda=' num2str(lambda)])
        tic
        ...
        recon_costs(Li,n) = sum((Xtrain_corr(:)-Xhat(:)).^2)/2;
        reg_costs(Li,n) = reg_cross;
        constraints(Li,n) = norm(constraint(:),1);
        ...
        tic
        [emds_W{Li,n}, emds_H{Li,n}, ids] = helper.similarity_WH_EMD(W, Htrain, W_hat, H_hat);
        num_detected{Li,n} = length(ids);
        ids_match{Li,n} = ids;
        toc
        ...
        num_significant{Li,n} = sum(is_significant);
    end
end
