...
if do_save
    save2pdf(sprintf('EMD_simulated_data_%s_extreme.pdf', helper.sanitize_name(data_type)))
end
...
if do_choose_lambda
    ...
    if do_save
        save2pdf(sprintf('Simulate_%s_extreme_choose_lambda_SeqNMF', helper.sanitize_name(data_type)))
    end
end
...
plot_MR(M, R)
if do_save
    save2pdf(sprintf('FlexMF_%s_extreme_demo_MR_lambda=%1.1e_lambdaM=%1.1e_lambdaR=%1.1e.pdf', ...
...
set(gcf, 'Units', 'normalized', 'Position', [0.1 0.1 0.8 0.8])
if do_save
    save2pdf(sprintf('Simulated_%s_extreme_result_SeqNMF.pdf', helper.sanitize_name(data_type)), gcf)
end
...
set(gcf, 'Units', 'normalized', 'Position', [0.1 0.1 0.8 0.8])
if do_save
    save2pdf(sprintf('EMD_Simulated_%s_extreme_data_FlexMF_lambda=%1.1e_lambdaM=%1.1e_lambdaR=%1.1e.pdf', ...
...
if do_save
    save2pdf(sprintf('EMD_Simulated_%s_extreme_data_compare_W.pdf', helper.sanitize_name(data_type)), gcf)
end
...
if do_save
    save2pdf(sprintf('EMD_Simulated_%s_extreme_data_compare_H.pdf', helper.sanitize_name(data_type)), gcf)
end
