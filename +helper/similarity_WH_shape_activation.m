function [S_shape, S_activation, ids, details] = similarity_WH_shape_activation(W, H, W_hat, H_hat, opts)
% similarity_WH_shape_activation
%
% Match motifs across W and W_hat, then calculate:
%   S_shape      : shift-tolerant motif-template similarity
%   S_activation : event-based activation similarity after opposite H shift
%
% Inputs:
...
%   opts.eventThreshold  : scalar threshold for event detection
%                          default = []
%                          If empty, threshold is set separately for each H
%                          as mean(H) + std(H).
%   opts.eventTol        : event matching tolerance in time bins
%                          default = 3
%   opts.minEventDistance: minimum distance between detected events
%                          default = 2
%   opts.beta            : beta for F_beta
%                          default = 1
%                          beta > 1 penalizes missing events more strongly.
%
% Outputs:
%   S_shape      : 1 x Khat vector, matched W similarity for each W_hat motif
%   S_activation : 1 x Khat vector, event-based F_beta for each matched H_hat motif
...
% ============================================================
% Step 2. Greedy motif matching based on S_shape
% ============================================================

S_shape = zeros(1, Khat);
S_activation = zeros(1, Khat);
ids = zeros(1, Khat);
