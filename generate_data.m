function [data,W,H,V_hat] = generate_data(T,Nneurons,Dt,NeuronNoise,SeqNoiseTime,SeqNoiseNeuron,shared,diff,stretch,bin,neg,seed)
%rng(2001)
if seed == 0
    rng shuffle
else
...
for ii = 1:Ktemp % go through each factor
    ...
    for jj = 1:sum(H(ii,:)) % go through each iteration of the sequence
        ...
        if neg > 0
            neg_indices = (rand(N,leng+150) < neg);
            tempW(neg_indices) = -tempW(neg_indices);
        end
