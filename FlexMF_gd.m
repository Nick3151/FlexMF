function [W, H, errors,grads,loadings,power] = FlexMF_gd(X, varargin)
%
% USAGE: 
%
...
for iter = 1 : params.maxiter
    ...
    if params.lambda>0
        ...
        WTXSHT = WTXS*H';
        dRegdH = params.lambda.*((~eye(K)).*sign(WTXSHT))*WTXS;  
    else 
        dRegdH = zeros(K,T); 
    end
    ...
    dLdH = dRecondH + dRegdH + dL1HdH + dHHdH; % include L1 sparsity, if specified
    grad_H_recon = mean(abs(dRecondH),'all');
    grad_H_WXH = mean(abs(dRegdH),'all');
    ...
    if ~params.W_fixed
        ...
        for l = 1 : L % could parallelize to speed up for long L
            ...
            if params.lambda>0 && params.useWupdate % Often get similar results with just H update, so option to skip W update
                dRegdW(:,:,l) = params.lambda.*XS*(H_shifted')*((~eye(K)).*sign(WTXSHT)); 
            end
            if params.lambdaOrthoW>0
                dWWdW(:,:,l) = params.lambdaOrthoW*Wflat*((~eye(K)).*sign(Wflat'*Wflat));
            end
        end
            
        dLdW = dRecondW + dRegdW + dL1WdW + dWWdW; % include L1 and Worthogonality sparsity, if specified
        grad_W_recon = mean(abs(dRecondW), 'all');
        grad_W_WXH = mean(abs(dRegdW), 'all');
        grad_W_WW = mean(abs(dWWdW), 'all');
        grad_W_L1W = mean(abs(dL1WdW), 'all');
        ...
        grads_W_recon_all = [grads_W_recon_all grad_W_recon];
        grads_W_WXH_all = [grads_W_WXH_all grad_W_WXH];
        grads_W_WW_all = [grads_W_WW_all grad_W_WW];
        grads_W_L1W_all = [grads_W_L1W_all grad_W_L1W];
...
%         tic
        ...
        beta = 0.8;
        etaW = 1;
        while helper.total_loss(W-etaW*dLdW,H,X,params) > helper.total_loss(W,H,X,params)-alpha*sqrt(sum(dLdW.^2,'all'))*etaW
            etaW = beta*etaW;
        end
        W = W-etaW*dLdW;
%         toc
        fprintf('Line search step on W: %f\n', etaW);
        etaW_all = [etaW_all etaW];
